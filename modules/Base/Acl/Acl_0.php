...
class Base_Acl extends Module
{
    ...
    /**
     * Looks like generic admin function.
     *
     * @todo it makes way too many udocumented things, further analysis required!
     */
    public function admin()
    {
        ...
        Base_ActionBarCommon::add('add', __('Add rule'), $this->create_callback_href(array(
            $this,
            'edit_rule'
        ), array(
            null,
        ...
        foreach ($perms as $p_id => $p_name) {
            ...
            $gb_row->add_data(array(
                'value' => _V($p_name),
                'class' => 'Base_Acl__permission',
                'attrs' => 'colspan="2"'
            ));
            $gb_row->no_actions();
            $perms = DB::GetAssoc('SELECT id, id FROM base_acl_rules WHERE permission_id=%d', array(
                $p_id
            ));
            foreach ($perms as $r_id) {
                $clearances = DB::GetAssoc('SELECT id, clearance FROM base_acl_rules_clearance WHERE rule_id=%d', array(
                    $r_id
                ));
                
                $gb_row = $gb->get_new_row();
                $gb_row->add_action($this->create_confirm_callback_href(__('Are you sure you want to delete this rule?'), array(
                    $this,
                    'delete_rule'
                ), array(
                    $r_id
                )), 'delete', __('Delete Rule'));
                $gb_row->add_action($this->create_callback_href(array(
                    $this,
                    'edit_rule'
                ), array(
                    $r_id,
    ...
    /**
     * Generate HTML interface for rule editing.
     *
     * @param int $r_id rule ID
     * @param int $p_id default NULL
     *
     * @todo it makes way too many udocumented things, further analysis required!
     */
    public function edit_rule($r_id, $p_id = null)
    {
        if ($this->is_back())
            return false;
        $counts            = 5;
        $all_clearances    = array(
            '' => '---'
        ) + array_flip(Base_AclCommon::get_clearance(true));
        $perms             = array(
            '' => '---'
        ) + DB::GetAssoc('SELECT id, name FROM base_acl_permission ORDER BY name ASC');
        $current_clearance = 0;
        ...
        if ($form->validate()) {
            ...
            if ($r_id !== null) {
                DB::Execute('DELETE FROM base_acl_rules_clearance WHERE rule_id=%d', array(
                    $r_id
                ));
            } else {
                if (!$p_id)
     ...
     *
     * @todo shoulndn't it return TRUE on succes and FALSE in case of bad parameter?
     * @param int $r_id rule ID
     * @return bool always false
     */
    public function delete_rule($r_id)
    {
	if (!is_numeric($r_id)) // sanity check
	    return false;
        DB::Execute('DELETE FROM base_acl_rules_clearance WHERE rule_id=%d', array(
            $r_id
        ));
        DB::Execute('DELETE FROM base_acl_rules WHERE id=%d', array(
            $r_id
        ));
        return false;
    }
